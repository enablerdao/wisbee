...
```bash
# ワンラインインストール
curl -fsSL https://raw.githubusercontent.com/enablerdao/wisbee/main/install.sh | bash

# または
wget -qO- https://raw.githubusercontent.com/enablerdao/wisbee/main/install.sh | bash
```
...
# スマートサーバーを起動（自動ポート検出）
python3 smart-server.py

# または特定のポートで起動
python3 smart-server.py 8888
```

ブラウザで表示されたURLを開く（通常 `http://localhost:8888`）

### 🌐 ポート管理システム

**スマートポート機能**により、ポート競合を自動回避：

- **推奨ポート番号**:
  - `8888` - Wisbee メインポート（Bee = 8888）
  - `7777` - Voice/リアルタイム機能
  - `9999` - 開発/プレミアム機能
  - `8899` - レガシーサポート
  - `11434` - Ollama API（固定）
...
- **自動ポート検出**:
  1. 優先順位: 8888 → 7777 → 9999 → 8899 → 8080 → 8000
  2. ポート使用中の場合は自動的に次のポートを試行
  3. 現在のポートは `.current_port` ファイルで確認可能
